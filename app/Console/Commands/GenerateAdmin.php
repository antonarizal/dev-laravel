...
class GenerateAdmin extends Command
{
     ...
     * @var string
     */
    protected $signature = 'generate:admin {name : The name of the view file} {--layout= : The layout to use for the view} {--force : Overwrite the view file if it exists} {--table= : Create an inline view}';
    ...
    public function handle()
    {
            ...
            if(!$this->option('force'))
              ...
              if($this->option('table')){
                ...
                $inputModal[$i] = $col != 'id' ? "<flux:ui.input.text name=\"$col\"  label=\"".Str::headline($col)."\" type=\"$type\" />" : '';
                $tableColumn[$i] = "<flux:ui.table.column label=\"".Str::headline($col)."\" field=\"$col\" :sortField=\"\$sortField\" :sortDirection=\"\$sortDirection\" sortable=true />";
                $tableRow[$i] = "<flux:ui.table.cell>{{ \$row->$col }}</flux:ui.table.cell>";
                  $i++;
                }
                $inputs = implode("\n",$input);
                $inputModals = implode("\n",$inputModal);
